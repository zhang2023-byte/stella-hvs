#!/usr/bin/env python3
"""Run one immutable terminal-only scratch dev or explicit test-smoke run."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-id",
        default=None,
        help="new run identity; default scratch-<UTC timestamp>",
    )
    parser.add_argument(
        "--variant",
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_id = args.run_id or "scratch-" + datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ"
    )
    load_env_files(ROOT)
    ...
    print(
        f"run summary: benchmark/scratch/hvs-extraction/runs/{run_id}/run_summary.json",
        flush=True,
    )
    return 0
...
def cli() -> int:
    try:
        ...
        print(f"scratch run refused: {exc}", file=sys.stderr, flush=True)
        return 2
