...
def infer_period_end(from_value: str, *, today: date | None = None) -> date:
    ...
    if DATE_RE.fullmatch(text):
        return min(date.fromisoformat(text), today)
    if MONTH_RE.fullmatch(text):
        year, month = (int(part) for part in text.split("-"))
        return min(date(year, month, calendar.monthrange(year, month)[1]), today)
    if YEAR_RE.fullmatch(text):
        year = int(text)
        return min(date(year, 12, 31), today)
    raise ValueError(f"--from must be YYYY-MM-DD, YYYY-MM, or YYYY; got {from_value!r}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("json_paths", nargs="*", type=Path, help="Explicit JSON paths such as notes/2026-03/2026-03.json.")
...
def main() -> int:
    ...
    selected_paths = list(args.json_paths)
    if args.from_value:
        try:
    ...
    if not selected_paths:
        raise SystemExit("Provide at least one JSON path, or use --from/--to to select note months.")
