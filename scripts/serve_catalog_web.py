...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=["static", "live"],
        default="static",
        help="Which site to serve. 'static' serves catalog/web/static/. "
...
def main() -> int:
    ...
    if args.mode == "static":
        static_dir = WORKSPACE / "catalog" / "web" / "static"
        if not static_dir.exists():
            raise FileNotFoundError(f"Static site not found: {static_dir}")
        os.chdir(static_dir)
        url = f"http://{args.host}:{args.port}/"
    else:
        live_dir = WORKSPACE / "catalog" / "web" / "live"
