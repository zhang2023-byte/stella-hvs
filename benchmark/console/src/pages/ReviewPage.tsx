...
export function ReviewPage() {
  ...
  const regression = group.scope === "regression";
  ...
  return (
    <div className="page review-page">
      <PageIntro eyebrow={`${regression ? "定向回归" : "运行复核"} · ${group.group_id}`} title={problems.length ? "先判断错误属于哪一类。" : "这个实验组无需异常复核。"} description={regression ? "定向回归只验证修复结果，不允许恢复、重试、评估或封存；仍有问题时修复工作流后新开回归实验。" : "外部服务传输故障可回到论文监控中重试；校验、复核、运行器或工作流错误必须先修复工作流，再新建实验。成功论文和已封存 Run 均不可重跑。"} actions={<button className="secondary-button" onClick={() => navigate(`/runs/${groupId}`)}>返回论文监控</button>} />
      {problems.length === 0 ? <section className="empty-state success-state"><span>✓</span><h2>全部实验已正常完成</h2><p>{regression ? "回归结果已经可供检查；稳定后请新建完整 10 篇正式 Dev 实验。" : "可以进入自动评估。评估前仍会执行泄漏审计和封存检查。"}</p>{!regression && <button className="primary-button compact-button" onClick={continueToEvaluation}>进入自动评估</button>}</section> : <>
        <section className="review-explainer"><div><span>1</span><strong>外部故障</strong><p>{regression ? "回归 Run 不重试；修复后新开回归实验。" : "仅确认由网络、限流或服务端导致的 transport_error 可重试。"}</p></div><div><span>2</span><strong>工作流错误</strong><p>修复代码后新建实验，不修改旧 Run。</p></div><div><span>3</span><strong>{regression ? "完成回归" : "按 unavailable 评估"}</strong><p>{regression ? "稳定后新建完整正式 Dev 实验。" : "明确接受缺失交付后，才允许进入评分。"}</p></div></section>
