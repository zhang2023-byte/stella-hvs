...
def evaluate_hvs_extraction_run(
    workspace: Path,
    run_id: str,
    *,
    gold_dir: Path,
    ...
    scorecard, private_details = score_run(
        ...
        run_label=run_id,
        run_source={
            "pipeline": "hvs_candidate_extraction",
            "run_id": run_id,
    ...
    details_dir = gold_dir.parent / "scoring-details" / "extraction" / run_id
    ...
    report = {
        "schema": schema_ref("hvs_extraction.evaluation"),
...
def render_terminal_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        f"HVS extraction dev evaluation: {report['run_id']}",
        "",
        "Delivery",
        (
