"""Deterministic projection from extraction artifacts to the scorer's input shape.

The extraction artifact is not a ``literature_hvs_candidates`` v0.2 artifact
...
def project_quantity(quantity: dict[str, Any] | None, *, coordinate: bool) -> dict[str, Any]:
    """Translate one extraction quantity to the scorer's empty-string conventions."""
...
def project_paper_result(result: dict[str, Any]) -> dict[str, Any]:
    """Project one extraction paper_result into a scorer-consumable AI document."""
    ...
    for entry in result.get("candidates") or []:
        ...
        # The extraction contract normalizes a reported percent to a 0-1
