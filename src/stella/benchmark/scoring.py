...
def _score_network_debug_run(
    *,
    ...
    """Score one finalized clean network debug run and its formal source."""

    debug_config = _load_json_object(
        run_dir / "debug_config.json", label="network debug config"
    )
    try:
        require_schema(
            debug_config, "benchmark.network_debug_config", require_current=True
        )
    except ValueError:
        raise ValueError(
            "network debug scoring requires the current debug config schema"
        )
    if debug_config.get("state") != "clean":
        raise ValueError("network debug scoring requires a finalized clean run")
    config_stable = {
        key: value
        for key, value in debug_config.items()
        if key != "content_sha256"
    }
    if canonical_sha256(config_stable) != debug_config.get("content_sha256"):
        raise ValueError("network debug config content hash does not match")
    debug_result = _load_json_object(
        run_dir / "debug_result.json", label="network debug result"
    )
    try:
        require_schema(
            debug_result, "benchmark.network_debug_result", require_current=True
        )
    except ValueError:
        raise ValueError(
            "network debug scoring requires the current debug result schema"
        )
    stable = {
        key: value
    ...
    if canonical_sha256(stable) != debug_result.get("content_sha256"):
        raise ValueError("network debug result content hash does not match")
    ...
    require_campaign_writable(str(campaign.get("campaign_id") or ""))
    expected_scope = "full_dev" if split == "dev" else "full_test"
    if debug_config["source_run"]["scope"] != expected_scope:
        raise ValueError(
            f"network debug scoring requires source scope={expected_scope} "
            f"for split={split}"
        )
    binding = debug_config.get("campaign") or {}
    if (
        binding.get("campaign_id") != campaign.get("campaign_id")
        or binding.get("manifest_sha256") != campaign_hash
    ):
        raise ValueError(
            "network debug campaign binding does not match campaign manifest"
        )
    if split == "test" and campaign.get("test_ready") is not True:
        raise ValueError("network debug test scoring requires a test-ready campaign")
    expected = papers_for_split(campaign, split)
    if debug_config.get("papers") != expected:
        raise ValueError("network debug papers do not match campaign split")

    source_binding = debug_config["source_run"]
    source_dir = run_dir.parent.parent / "runs" / source_binding["run_id"]
    source_config_path = source_dir / "run_config.json"
    if sha256_file(source_config_path) != source_binding.get("run_config_sha256"):
        raise ValueError("network debug source run config hash does not match")
    source_config = _load_json_object(
        source_config_path, label="source run config"
    )
    require_schema(source_config, "benchmark.run_config", require_current=True)
    if (
        source_config.get("method_fingerprint")
        != debug_config.get("method_fingerprint")
    ):
        raise ValueError("network debug method fingerprint does not match source run")
    if source_config.get("papers") != expected:
        raise ValueError("network debug source run papers do not match split")

    test_release: dict[str, str] | None = None
    if split == "test":
        if releases_root is None:
            raise ValueError("test debug scoring requires a release manifest root")
        ...
        if release_path is None:
            raise ValueError("test debug scoring requires a matching release manifest")
        test_release = {
            "sha256": sha256_file(release_path),
            "path": str(release_path),
        }

    l1, ledger = _debug_delivery_ledger(run_dir, expected)
    core_delivery = {
        "papers": {
            "valid": list(l1["complete"]),
            "invalid": list(l1["failed"]),
            "missing": list(l1["missing"]),
        },
    ...
    if pricing_snapshot["snapshot_id"] != debug_config["pricing_snapshot"][
        "snapshot_id"
    ]:
        raise ValueError(
            "pricing snapshot must match the network debug frozen snapshot"
        )
    estimated_cost = estimate_api_cost(
        snapshot=pricing_snapshot,
        snapshot_path=pricing_snapshot_path,
        run_config=source_config,
        usage=debug_result["usage"],
    )
    ...
    default_label = (
        f"{debug_config['debug_run_id']}--gold-{gold_snapshot['selection_id']}"
    )
    ...
    primary, private_details = score_run(
        ...
        run_label=label,
        run_source={
            "mode": "formal_campaign_network_debug",
            "run_id": debug_config["debug_run_id"],
        },
        bootstrap_iterations=bootstrap_iterations,
        bootstrap_seed=bootstrap_seed,
    )
    ...
    pseudo_manifest = {
        ...
        "l2_core_field_delivery": ledger["l2"],
        "l0": {"format_validation": debug_result["format_validation"]},
    }
    primary["l0"] = _l0_scorecard_block(
        manifest=pseudo_manifest,
        expected=expected,
        integrity_checks=[
            "debug_result_content_hash",
            "source_run_config_hash",
    ...
    primary["formal"] = {
        ...
        "run_manifest_sha256": source_binding.get("run_manifest_sha256"),
        "method_fingerprint": debug_config["method_fingerprint"],
        "test_release": test_release,
    }
    primary["network_debug"] = {
        "debug_run_id": debug_config["debug_run_id"],
        "source_run_id": source_binding["run_id"],
        "source_scope": source_binding["scope"],
        "debug_result_sha256": debug_result["content_sha256"],
        "retry_commands": debug_result.get("retry_commands"),
        "recovered_papers": sorted(
            paper["arxiv_id"]
            for paper in debug_result.get("papers", [])
            if paper.get("origin") == "recovered"
        ),
        "terminal_network_check": debug_result["terminal_network_check"],
    }
    component_hashes = dict(
        (debug_config.get("component_hashes") or {}).get("current") or {}
    )
    primary["provenance"] = {
        "evaluation_label": label,
        "scorer_sha256": component_hashes.get("scorer"),
        "identity_matching_sha256": component_hashes.get("identity_matching"),
        "unit_table_sha256": component_hashes.get("unit_table"),
        "gold_snapshot": {
...
def score_formal_campaign_run(
    *,
    ...
    if (run_dir / "debug_config.json").is_file():
        return _score_network_debug_run(
            campaign_path=campaign_path,
            split=split,
            run_dir=run_dir,
            gold_dir=gold_dir,
            gold_manifest_path=gold_manifest_path,
            gold_selection_path=gold_selection_path,
