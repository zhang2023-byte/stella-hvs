...
from stella.benchmark.scratch.bibliography import discover_bibliography
from stella.benchmark.scratch.cleaning import (
    render_manuscript_view,
    strip_tex_comments,
)
from stella.benchmark.scratch.ecsv import SelectedEcsv, select_ecsv_tables
from stella.benchmark.scratch.method_config import ScratchContextBudget
from stella.benchmark.scratch.tex_graph import TexGraphError, resolve_tex_graph
...
RUNS_RELATIVE_DIR = Path("benchmark/scratch/hvs-extraction/runs")
...
def build_prepared_input(
    ...
    *,
    ...
    base: dict[str, Any] = {
        "schema": schema_ref("benchmark.hvs_extraction_scratch.prepared_input"),
...
def write_prepared_input(workspace: Path, run_id: str, artifact: dict[str, Any]) -> Path:
    """Atomically persist one prepared_input artifact under the scratch tree."""
