"""Dev/test selection and read-only preflight for immutable scratch runs."""
...
from stella.benchmark.scratch.run import validate_scratch_run_id
...
def ensure_run_available(workspace: Path, run_id: str) -> None:
    ...
    if (run_root.parent / "locks" / f"{run_id}.lock").exists():
        raise FileExistsError(f"scratch run lock already exists: {run_id}")
