...
`InclusionAssessment`, and `CoordinateQuantityRecord`. The schema reference is generated by
...
7. Fill `inclusion_assessment` and classify `candidate_origin`:
   - `paper_labels[]` records non-exclusive wording used by the paper.
   - `galactic_bound_claim` records the paper's mutually exclusive Milky Way
     bound/unbound claim.
   - `inclusion_basis` records why Stella includes the object.
   - `extraction_confidence` is `high`, `medium`, or `low` agent extraction
     confidence with a required `confidence_reason`; it is not a physical
     probability.
...
9. For every candidate, fill standard `core` and typed v7 fields where the
   ...
   `method_chain_refs`.
   Use `photometry[]`, `spectroscopy[]`, `stellar_parameters`, `abundances[]`,
   `quality_flags[]`, `orbit`, and `astrophysical_origin` before `extra[]`.
   `extra[]` is only for paper-specific fields that do not fit typed groups.
...
- Do not silently drop missing 6D components; leave the field absent and mention
  the gap in `inclusion_assessment` or `extraction.summary`.
...
Every quantity in `core`, typed candidate groups, and `extra[]` must include both:
...
0-1 fraction and leave `unit` empty. Preserve the paper-visible text in
...
`unit: ""`. Origin/model probabilities, p-values, and likelihood ratios such
as `p_MW`, `p_LMC`, and `log(L_LMC/L_MW)` belong in
`astrophysical_origin.hypothesis_metrics[]`, not in core bound assessment.
...
Every value in `core`, typed candidate groups, and `extra[]` must include `source_refs`.
