...
方法链、字段定义、引用来源和缺失说明由原文行号支撑。候选标识统一写在
`identifiers` 下：`record_id` 使用 `<arxiv_id>:cand-001` 等 Stella 内部记录号，
`paper_candidate_id` 是论文内首选展示名，`gaia_source_id` 是空字符串或严格
`Gaia DR3/EDR3/DR2 ...` 机器标识，`all[]` 收录论文中出现过的所有名称、编号和 Gaia
source ID 并逐项保存 `source_refs`；`record_id` 不进入 `all[]`。`method_chain[]` 使用
