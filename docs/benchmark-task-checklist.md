...
- ✅ 分层抽样 manifest（框 207 → 抽 47：全 PDF-only 专家标注；种子 20260611 字节级可复算）
- ✅ GUIDELINE.md（英文）
- ✅ gold 轻量 schema + YAML 模板 + 升格脚本（唯一可写 `benchmark/gold/` 的代码）
- ✅ 专家标注表单（全样本 PDF-only，直写 `benchmark/gold/` YAML/JSON）
- ✅ 防污染三规则写入 AGENTS.md + 静态测试强制
- ✅ 用户审核余项全部通过（2026-06-14）：名单盖章放行、模板试填正/负两分支验通过、README、防污染三规则确认；试填挤出的 GUIDELINE 待补 4 条 + 模板 1 处小改归入 Phase 3 校准（详见自动记忆时间线）
...
- ✅ 正式 run 模型清单已定（deepseek-v4-pro ×3 测方差 + mimo-v2.5-pro ×1 横评）
