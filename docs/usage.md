...
--max-jobs N               --jobs Auto 的上限，默认 12 或 STELLA_MAX_CATALOG_JOBS
...
- 全量重跑可以用 `--jobs Auto` 按论文并行；Auto 会按论文数选择 1/2/4/8/12 个 worker，并受 `--max-jobs` 或 `STELLA_MAX_CATALOG_JOBS` 限制。并行不会减少 API/网络总成本，只是减少等待时间；外部资源和 Agent locator 较多时收益最明显。DeepSeek API 并发限额是动态的，如果出现 429、timeout 或大量 `agent_error`，下次调低 `--max-jobs`。
