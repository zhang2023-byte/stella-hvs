...
# 修订记录（2026-07-05，用户批准）

以下修订经用户在 2026-07-05 的盲区评审与执行计划讨论中拍板，取代本文正文
中与之冲突的表述；正文保留原样作为历史决策记录。

1. **Phase 3 标注协议**："47 篇全 PDF-only 人工标注、不设 AI 预填"修订为
   **专家先决 + AI 誊抄**（`expert_led_scribe.v1`，规范文本为
   `benchmark/GUIDELINE.md` §2）：专家先独立读 PDF 完成全部 L1 判断与数据
   定位；誊抄 agent 仅读同一 PDF 做机械誊抄，不得增删或重释候选；专家逐值
   核对后保存。判断型选择（多值选一、字段词表映射、限值语义、概率归一）
   由专家指定或确认。信息流向是专家 → agent，L1 锚定被设计规避，故不设
   双流程对照实验。`annotation_process` 块记录誊抄工具；纯手工标注依旧
   有效。
2. **15 候选 L2 截断规则删除**（原 GUIDELINE §3）：誊抄协议使转录成本可
   忽略，L2 全量记录；既有 8 篇正式标注最多 9 候选、从未触发旧规则，评分
   器无需截断处理逻辑。
3. **gold 外置到私有仓库**：`benchmark/gold/` 与含金标值的
   `benchmark/comparison/` HTML 迁至私有仓库（工作区经 `STELLA_GOLD_DIR`
   访问），公共仓保留 SHA256 完整性清单
   （`benchmark/manifest/gold_manifest.json`）。公共仓历史不重写、不
   force push；已在公开历史中暴露的 8 篇校准期标注在论文中作脚注标记。
   升格生成的 JSON 双生自动携带确定性 canary 供 run 泄漏审计。动机：仓库
   公开且联网 agent 可达，静态白名单测试不构成运行时隔离。
4. **B2 红线修订**："不自建 agent 框架"解禁——三方法对比（A：skill 通用
   coding agent，不可复现上界参考；B：直接 API 工作流，基线；C：自研轻量
   提取智能体，论文方法主张）进入论文计划；方法 C 在 B 管线基础上演化
   （逐字段工具填写 + 独立审核者 → 显式状态机，内层自主性可开关），不引
