...
| `benchmark/campaigns/<campaign_id>/runs/<run_id>/` | Local ignored AI run archive, including per-paper extraction, context manifest, report, reviewer challenge record, shared-roster copy, and request/response attempts | Written by the selected extraction workflow; never read gold or mutate sealed runs |
| `benchmark/campaigns/<campaign_id>/runs/_shared_rosters/<shared_key>/` | Local ignored surface-neutral roster cache for Methods B/C | Keyed by method, paper, extractor model/provider, roster prompt/rules, context, and code version; FULL/CORE may share only under an identical key, and each consuming Run receives its own bundle/attempt copy |
...
| `logs/benchmark-dev-console/<campaign_id>/_groups/<group_id>/evaluation/` | Local ignored dev-console evaluation state and aggregate scorecards | Owns local audit/seal/score orchestration and browser-safe aggregate cards; private item details are excluded |
