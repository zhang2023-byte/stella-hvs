...
| `benchmark/campaigns/<campaign_id>/runs/<run_id>/` | Local ignored AI run archive, including per-paper extraction, context manifest, report, reviewer challenge record, shared-roster copy, and request/response attempts | Written by the selected extraction workflow; sealed run-manifest v3 records decoupled CORE (`full_core`/`core_prov`) and enrichment (`full_enrichment`) delivery status and hashes, with enrichment validity never demoting a valid core; never read gold or mutate sealed runs |
| `benchmark/campaigns/<campaign_id>/runs/_shared_rosters/<shared_key>/` | Local ignored surface-neutral roster cache; new runs use Method B/Core, historical Method C/FULL entries remain readable | Keyed by method, paper, extractor model/provider, roster prompt/rules, roster-reviewer model/provider/prompt/rules, context, and code version; legacy FULL/CORE may share only under an identical key, each bundle seals exactly one pre-hash roster review (`accepted`/`revised` with reviewer provenance), and each consuming Run receives its own bundle/attempt copy |
