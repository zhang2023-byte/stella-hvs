...
Each `catalog/candidates/<object_id>.json` uses `schema_version: stella.hvs_candidate_catalog.object.v4` and contains:
...
`external_enrichment` is generated by the default `--enrichment-mode auto` merge behavior. It queries public SIMBAD and Gaia DR3 TAP services through `astroquery`, and it can be disabled with `--enrichment-mode off` or made strict with `--enrichment-mode required`. Enrichment values do not overwrite paper-level quantities and do not affect object grouping; conflicts appear as enrichment warnings for review.

Merge rules:

- Same normalized Gaia source ID: merge. `Gaia EDR3 <source_id>` and `Gaia DR3 <source_id>` are the same DR3-family match key, while DR2 remains separate.
...
`catalog/03_hvs_candidates_index.json` summarizes object count, source count, merge warnings, enrichment statuses, enrichment warnings, skipped inputs, and each object link. `catalog/03_hvs_candidates_index.md` is the generated reading view. For strict rebuilds, run `scripts/merge_hvs_candidate_catalog.py ... --fail-on-skipped` so malformed paper-level inputs fail the command instead of only appearing in `skipped[]`; add `--enrichment-mode required` when SIMBAD/Gaia enrichment failures should also fail the command.
...
- Each object's canonical identifier, object JSON path, source count, Gaia source IDs, paper candidate IDs, and enrichment status.
- Merge warnings, enrichment warnings, and skipped input files.

`catalog/03_hvs_candidates_index.md` emphasizes:

- Each object's JSON link and source count.
- Object-level Gaia/paper identifiers.
- Merge and enrichment warnings that need manual review.
