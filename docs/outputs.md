...
| `benchmark/campaigns/<campaign_id>/runs/_shared_rosters/<shared_key>/` | Local ignored surface-neutral roster cache for Methods B/C | Keyed by method, paper, extractor model/provider, roster prompt/rules, roster-reviewer model/prompt/rules, context, and code version; FULL/CORE may share only under an identical key, each bundle seals exactly one pre-hash roster review (`accepted`/`revised` with reviewer provenance), and each consuming Run receives its own bundle/attempt copy |
