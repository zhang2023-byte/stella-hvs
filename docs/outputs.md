...
Each `catalog/<object_id>.json` uses `schema_version: stella.hvs_candidate_catalog.object.v2` and contains:
...
`catalog/03_hvs_candidates_index.json` summarizes object count, source count, warnings, skipped inputs, and each object link. `catalog/03_hvs_candidates_index.md` is the generated reading view. For strict rebuilds, run `scripts/merge_hvs_candidate_catalog.py ... --fail-on-skipped` so malformed paper-level inputs fail the command instead of only appearing in `skipped[]`.
...
- `html/live/`: under a local HTTP server, reads `catalog/03_hvs_candidates_index.json` and each `catalog/<object_id>.json` live. Refreshing the page reflects catalog updates.
