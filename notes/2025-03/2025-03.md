...
- 观测 catalog 判定：已判定 3 篇；疑似包含真实观测 catalog/样本 1 篇；方法 `llm` / `deepseek-chat`
...
- 观测 catalog 判定：是；role=followup_observations；scope=multiple_objects；confidence=0.95；Paper reports results of long-term spectroscopic monitoring of 188 runaway star candidates, providing precise radial velocities and orbital solutions.
- 可能数据产品：source_ids, radial_velocities, spectra, candidate_table
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.9；Pure simulation study of 28,000 simulated HVS trajectories to assess impact of Galactic bar and LMC. No observational data or real stellar objects analyzed.
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.85；N-body simulations of star cluster mergers producing runaway and high-velocity stars. Theoretical/numerical study without observational data.
