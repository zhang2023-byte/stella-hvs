...
- 观测 catalog 判定：已判定 3 篇；疑似包含真实观测 catalog/样本 0 篇；方法 `llm` / `deepseek-chat`
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.95；Purely theoretical modeling paper on synchrotron emission and polarization in runaway star bowshocks using MHD simulations. No observational data or object lists presented.
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.9；Simulation paper using N-body and Monte Carlo methods to study runaway stellar collisions forming VMS and IMBH. No observational stellar data or catalogs.
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.85；Theoretical study of disruption mechanisms producing hypervelocity stars in Milky Way center using dynamical modeling. Mentions HVS production but presents no observational catalog or object-level data.
- DeepXiv brief：弱相关条目未拉取；仅保留 search 阶段元数据
