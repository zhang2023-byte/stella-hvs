...
- 简介生成：DeepXiv `brief` 仅用于强相关/直接相关；本月拉取 3 篇，弱相关跳过 3 篇
- 去重后候选：31 篇；标题复核通过：6 篇
- 清洗统计：分类过滤 0 篇；score 过滤 0 篇；标题复核过滤 25 篇
- 规则分层：直接相关 3 篇；弱相关 3 篇；弱相关 LLM 复核 3 篇；复核后保留 3 篇
- 观测 catalog 判定：已判定 6 篇；疑似包含真实观测 catalog/样本 5 篇；方法 `llm` / `deepseek-chat`
...
- 观测 catalog 判定：是；role=new_catalog；scope=multiple_objects；confidence=0.95；Paper identifies 149 hypervelocity star candidates from Gaia DR3 and presents follow-up spectroscopy for 23 targets. Provides object-level data: source selection, velocity estimates, orbital origins.
- 可能数据产品：source_ids, coordinates, astrometry, radial_velocities, candidate_table, orbit_integrations
...
- 观测 catalog 判定：是；role=uses_existing_catalog；scope=multiple_objects；confidence=0.85；Uses DESI DR1 and Gaia DR3 data to search for ejected stars from Galactic Centre. Analysis is based on observational catalog data (metallicity, angular momentum) to place constraints on ejection rates.
- 可能数据产品：coordinates, astrometry, radial_velocities, abundances
...
- 观测 catalog 判定：否；role=not_catalog；scope=none；confidence=0.9；Purely theoretical/simulation paper. Uses Monte Carlo N-body simulations and particle spray codes to model ejection from globular clusters. No observational stellar data presented.
...
- 命中关键词：`runaway stars`
- 命中分类：`astro-ph.GA`
- 标题复核：rule-weak-llm-confirmed，confidence=0.95，Weak title match: runaway-related mechanisms LLM confirmed: Explicitly studies a runaway Wolf-Rayet star with measured peculiar velocities (50.7 km/s) and kinematics analysis of its motion and shell origin.
- 观测 catalog 判定：是；role=followup_observations；scope=single_object；confidence=0.9；Presents radio continuum detection and analysis of a single runaway Wolf-Rayet star (WR16). Includes Gaia parallax, proper motions, peculiar velocities, and shell kinematics.
- 可能数据产品：coordinates, astrometry, spectra, orbit_integrations
...
- 命中关键词：`hypervelocity stars`, `high-velocity stars`, `runaway stars`, `unbound stars`, `escaping stars`
- 命中分类：`astro-ph.GA`
- 标题复核：rule-weak-llm-confirmed，confidence=0.85，Weak title match: high proper-motion or unusual kinematics LLM confirmed: Focuses on stars with slow/retrograde velocities, explicitly discusses dynamical ejection from clusters as a mechanism, which is relevant to runaway/high-velocity star origins.
- 观测 catalog 判定：是；role=uses_existing_catalog；scope=multiple_objects；confidence=0.8；Identifies and investigates ~70 stars with thin-disk abundances and unusual kinematics. Analysis based on existing observational data (abundances, velocities) to study ejection mechanisms.
- 可能数据产品：coordinates, astrometry, radial_velocities, abundances
- DeepXiv brief：弱相关条目未拉取；仅保留 search 阶段元数据
...
- 命中分类：`astro-ph.GA`
- 标题复核：rule-weak-llm-confirmed，confidence=0.9，Weak title match: runaway-related mechanisms LLM confirmed: Paper explicitly studies bowshock stars, reports a runaway fraction of 23%, and analyzes kinematics of runaways with v_2D ≥ 25 km/s. Directly addresses origins and binary nature of runaway stars.
- 观测 catalog 判定：是；role=new_catalog；scope=multiple_objects；confidence=0.95；Presents fundamental parameters for 103 bowshock nebula central stars. Includes new spectroscopic observations, stellar parameters, and identifies 19 runaway stars with measured 2D velocities.
- 可能数据产品：source_ids, coordinates, spectra, abundances, candidate_table
- DeepXiv brief：弱相关条目未拉取；仅保留 search 阶段元数据
