"""Anti-drift guard: extraction code must match the frozen decisions verbatim.

Compares the canonical extraction rule text (D054) and the frozen prompt
templates (D009, D024 as amended by D052, D033) in
``benchmark/hvs_candidate_extraction_decisions.yaml`` against the implemented
...
class FrozenPromptTemplateTest(unittest.TestCase):
    ...
    def test_extractor_json_object_variant_matches_d057(self) -> None:
        ...
        rules = roster_prompts.render_rule_profile(ROOT, "hvs_candidate_roster", "prompt")
