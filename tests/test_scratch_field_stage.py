...
from stella.benchmark.scratch.field_stage import (
...
def budget(limit: int = 900000) -> ScratchContextBudget:
...
def frozen_config() -> ScratchMethodConfig:
    ...
    return ScratchMethodConfig(
        ...
        components=ScratchComponentHashes(
            rule_profile_sha256={"hvs_field_extractor_scratch_tex_ecsv": "a" * 64},
...
def make_workspace(tmp: str, *, field_limit: int = 900000, roster_candidates: list[dict] | None = None) -> Path:
    ...
    roster_final = {
        "schema": {"name": "benchmark.hvs_extraction_scratch.roster_final", "version": 1},
...
class FieldStageTest(unittest.TestCase):
    ...
    def test_tex_only_mode_uses_tex_profile_and_no_ecsv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                artifact["provenance"]["rule_profile"], "hvs_field_extractor_scratch_tex"
            )
            for call in transport.calls:
