...
def budget(limit: int, reserve: int = 0) -> ScratchContextBudget:
...
class BuildPreparedInputTest(unittest.TestCase):
    def test_prepared_artifact_sections(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                artifact["schema"],
                {"name": "benchmark.hvs_extraction_scratch.prepared_input", "version": 1},
            )
    ...
    def test_write_prepared_input_roundtrip_under_scratch_tree(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                f"benchmark/scratch/hvs-extraction/runs/run-test/prepared_inputs/{ARXIV_ID}.json",
            )
