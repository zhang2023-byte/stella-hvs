...
class DetectPeerConsistencyFlagsTest(unittest.TestCase):
    ...
    def test_shared_peer_evidence_flags_the_null_candidate(self) -> None:
        ...
        self.assertEqual(flag["path"], "$.core.bound_assessment.bound_probability")
        self.assertEqual(flag["peers"], 2)
        self.assertIn("main.tex lines", flag["locator"])

    def test_no_flag_when_all_peers_filled(self) -> None:
        self.assertEqual(self.flags_for(self.submissions(third_null=False)), {})
    ...
    def test_failed_candidates_never_participate(self) -> None:
        artifacts = {
            "candidate-001": artifact("candidate-001", self.submissions(third_null=True)[0]),
            "candidate-002": {
                "record_id": "candidate-002",
                "status": "field_extraction_failed",
                "fields": None,
            },
            "candidate-003": artifact("candidate-003", fixtures.group_null_probability_submission()),
        }
        self.assertEqual(
...
class PeerReviewFieldStageTest(unittest.TestCase):
    def transport_handler(self, names, *, review_payloads):
        ...
        def handler(kwargs: dict) -> dict:
            ...
            if review is not None:
                ...
                payload_factory = review_payloads[names[-1]]
                return fake_response(payload_factory())
            block = kwargs["messages"][1]["content"].split(
                "===== BEGIN ASSIGNED CANDIDATE =====", 1
            )[1].split("===== END ASSIGNED CANDIDATE =====", 1)[0]
    ...
    def test_review_populates_shared_group_field(self) -> None:
        names = ["HVS-A", "HVS-B", "HVS-C"]
        state, handler = self.transport_handler(
            names,
            review_payloads={
                "HVS-C": lambda: fixtures.group_bound_probability_submission(
                    member_line=8
                )
            },
        )
        ...
        def verify(workspace: Path, transport: RecordingTransport) -> None:
            ...
            self.assertIn(
                "$.core.bound_assessment.bound_probability",
                state["review_messages"][0],
            )
    ...
    def test_drifting_review_is_rejected(self) -> None:
        names = ["HVS-A", "HVS-B", "HVS-C"]

        def drifting_payload() -> dict:
            payload = fixtures.group_bound_probability_submission(
                member_line=8
            )
            radial = fixtures.quantity(
                value="900",
                unit="km/s",
                direct_evidence=[
                    {
            ...
            payload["core"]["observed_phase_space"]["radial_velocity"] = radial
            return payload

        state, handler = self.transport_handler(
            names, review_payloads={"HVS-C": drifting_payload}
        )
        ...
        def verify(workspace: Path, transport: RecordingTransport) -> None:
            ...
            self.assertIsNone(
                artifact["fields"]["core"]["observed_phase_space"][
                    "radial_velocity"
                ]
            )
            (repair,) = artifact["repair_history"]
            self.assertEqual(repair["final_status"], "correction_drift")
    ...
    def test_renamed_members_trigger_identically(self) -> None:
        ...
        state, handler = self.transport_handler(
            names,
            review_payloads={
                "S-1": lambda: fixtures.group_bound_probability_submission(
                    member_line=8
                )
            },
        )
