"""HVS extraction-to-scorer projection tests (D034, D038)."""
...
from stella.hvs_extraction.core_document import build_core_document, empty_core
from stella.hvs_extraction.projection import (
...
class CoreDocumentTest(unittest.TestCase):
    def test_field_failure_preserves_l1_candidate_with_null_core(self) -> None:
        result = paper_result(
            status="partial",
            candidates=[
                {
                    ...
                    "failure": {
                        ...
                        "detail": "three requests were used",
                    },
                }
            ],
        )
        ...
        self.assertEqual(candidate["field_status"], "field_extraction_failed")
        self.assertEqual(candidate["core"], empty_core())
        self.assertEqual(
            candidate["failure"]["code"], "request_budget_exhausted"
        )
        self.assertNotIn("method_chain", document)
        self.assertNotIn("photometry", candidate)

    def test_complete_candidate_keeps_native_evidence(self) -> None:
        result = paper_result(
            candidates=[{"record_id": "candidate-001", "status": "fields_complete"}],
            fields=complete_fields(),
        )
        ...
        document = build_core_document(
            result, campaign_id="hvs-extraction-v5"
        )
        quantity = document["candidates"][0]["core"]["observed_phase_space"][
            "radial_velocity"
        ]
        self.assertEqual(
            quantity["direct_evidence"][0]["source"]["quantity_raw_value"],
            "805",
        )
